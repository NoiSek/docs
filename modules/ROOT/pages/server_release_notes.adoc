...
Dear ownCloud administrator, please find below the changes and known issues in ownCloud Server 10.9 that need your attention. You can also read {owncloud-changelog-url}[the full ownCloud Server changelog] for further details on what has changed.

=== Migrations

* To improve the performance of addressbook search queries (e.g., when looking for federated users to share with), a migration step adds an index in the `cards_properties` table. The impact on upgrade duration depends on the number of known federated users. https://github.com/owncloud/core/pull/39017[#39017]
* A migration step takes care to set the new option `allow_user_to_change_mail_address` according to what has been set in `allow_user_to_change_display_name` to make sure that there's no unintentional behavior change after upgrading. The impact on upgrade duration is insignificant.  https://github.com/owncloud/core/pull/39288[#39288]
...
Version 10.9 adds support for MariaDB 10.6 (https://github.com/owncloud/core/pull/39286[#39286]). If you're planning a database update to a MariaDB version > 10.5, it is very important to prepare the ownCloud installation first:

1. Update ownCloud Server to version 10.9
2. Run `occ db:convert-row-format` (this can take some time depending on ???)
3. Update MariaDB to version 10.6

More information can be found in the [ownCloud documentation]

=== Highly Improved Initial Sync and Discovery Performance

Server 10.9 comes with the means to drastically reduce the time needed by the ownCloud Clients for the initial discovery of the contents in user accounts, especially for those with many files and huge directory structures. Practically, this means when a user account is initially set up, e.g., in the ownCloud Desktop Client or when larger directory structures are added to an existing account (e.g., via sharing or external storage mounts like Windows Network Drives), the client does not need to check every particular folder anymore, as it used to be previously. Instead, the server will compose a full content listing and provide it to the client ("Streaming PROPFIND with depth=infinity", https://github.com/owncloud/core/pull/38583[#38583]). Additionally, to prevent memory issues on the server side, this content listing will be streamed to the client while it is being created.

This new server-side capability needs changes in the ownCloud Clients to make use of it. At the time of the ownCloud Server 10.9 release, client implementations for iOS, Android and Desktop are in the making but not yet available. The implementations will follow soon with the next client releases.
...
In certain scenarios it is not desired that all users of an ownCloud installation can create public links and share data publicly, respectively. To cover for this, ownCloud Server 10.9 comes with a new configuration option for administrators that allows to restrict the creation of public links to users in certain groups (https://github.com/owncloud/core/pull/38980[#38980]).
...
In some cases it is necessary to manage storage locations of users, e.g., if you want to distribute them across multiple storages. To facilitate this process, Server 10.9 introduces a new _occ_ command (`occ user:move-home`) that allows to move user home folders to a new location (https://github.com/owncloud/core/pull/39136[#39136]). The command takes care of moving the physical data and takes care of updating all other related information.
Administrators can find more information in the LINK[ownCloud documentation].
...
* https://marketplace.owncloud.com/apps/web[Web] 4.x.x
* Windows Network Drive 2.1.0
* TBC

=== Other Notable Changes

* The handling for inaccessible federated shares has been improved to better avoid removing temporarily unavailable shares. https://github.com/owncloud/core/pull/38474[#38474]
* Group admins can now add users to their groups via the HTTP Provisioning API (previously this was only possible via the web-based user management). https://github.com/owncloud/core/pull/39013[#39013]
* Issues with updating apps via the Market app under certain circumstances have been resolved. https://github.com/owncloud/core/pull/39108[#39108]
...
* The "Home" icon in the breadcrumb of the Files app has been replaced with the icon and text of the respective view (e.g., All Files, Deleted Files) so that its functionality is more clear to the user. https://github.com/owncloud/core/pull/39367[#39367]
...
* The group listing of the user management in the web interface has been improved in regards of usability. https://github.com/owncloud/core/pull/39262[#39262]
...
All xref:server_release_notes.adoc#known-issues[known issues from Server 10.8] have been fixed.
